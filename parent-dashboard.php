...
// Get appointments
// Left-join the most recent pending modification request so the UI can show
// "Edit pending" without a second query.
// Subquery picks the most recent PENDING request per appointment so the join
// never multiplies appointment rows even if duplicate requests slipped in.
$appointments_query = "SELECT a.*, p.first_name as child_first_name, p.last_name as child_last_name,
                              u.first_name as doctor_first_name, u.last_name as doctor_last_name,
                              amr.id as pending_modification_id,
                              amr.new_date as pending_new_date,
                              amr.new_time as pending_new_time,
                              amr.new_type as pending_new_type
                       FROM appointments a
                       JOIN patients p ON a.patient_id = p.id
                       JOIN users u ON a.doctor_id = u.id
                       LEFT JOIN appointment_modification_requests amr
                              ON amr.id = (
                                  SELECT MAX(amr2.id) FROM appointment_modification_requests amr2
                                  WHERE amr2.appointment_id = a.id AND amr2.status = 'PENDING'
                              )
                       WHERE p.parent_id = ?
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_child'])) {
        ...
        if (handleEditAppointment($conn, $user_id)) {
            $_SESSION['success_message'] = 'Edit request submitted. The doctor will review and approve before changes are applied.';
        }
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } elseif (isset($_POST['cancel_appointment'])) {
        if (handleCancelAppointment($conn, $user_id)) {
            $_SESSION['success_message'] = 'Cancellation request submitted. The assigned doctor will review and confirm.';
        }
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
...
function handleEditAppointment($conn, $user_id) {
    ...
    $verify = "SELECT a.id, a.doctor_id, a.status, a.appointment_date FROM appointments a
               JOIN patients p ON a.patient_id = p.id
    ...
    // Edits must be submitted before the appointment date so the doctor has
    // time to review the request.
    if (strtotime($row['appointment_date']) < strtotime(date('Y-m-d'))) {
        $_SESSION['error_message'] = 'Past appointments can no longer be modified.';
        return false;
    }
    ...
    // Reject if a PENDING modification already exists for this appointment —
    // doctors should resolve the current request before another is queued.
    $pending_chk = mysqli_prepare($conn,
        "SELECT id FROM appointment_modification_requests WHERE appointment_id = ? AND status = 'PENDING' LIMIT 1");
    mysqli_stmt_bind_param($pending_chk, "i", $appointment_id);
    mysqli_stmt_execute($pending_chk);
    if (mysqli_num_rows(mysqli_stmt_get_result($pending_chk)) > 0) {
        $_SESSION['error_message'] = 'You already have a pending edit request for this appointment. Please wait for the doctor to review it.';
        return false;
    }

    // Parent-initiated edits require doctor approval — record the request and
    // notify the assigned doctor; the appointments table is NOT updated until
    // the doctor approves.
    $insert = "INSERT INTO appointment_modification_requests
               (appointment_id, requested_by, new_date, new_time, new_type, new_reason, status)
               VALUES (?, ?, ?, ?, ?, ?, 'PENDING')";
    $stmt = mysqli_prepare($conn, $insert);
    mysqli_stmt_bind_param($stmt, "iissss",
        $appointment_id, $user_id, $appointment_date, $appointment_time, $appointment_type, $reason);
    if (mysqli_stmt_execute($stmt)) {
        $when = date('M j, Y', strtotime($appointment_date)) . ' at ' . date('h:i A', strtotime($appointment_time));
        send_appointment_notification($conn, $user_id, 'Edit request submitted',
            "Your request to reschedule the appointment to $when has been submitted. The doctor will review and confirm.",
            $appointment_id);
        send_appointment_notification($conn, intval($row['doctor_id']), 'Appointment edit requested',
            "A parent has requested to reschedule an appointment to $when. Please review and approve or reject.",
            $appointment_id);
        foreach (get_admin_user_ids($conn) as $aid) {
            send_appointment_notification($conn, $aid, 'Appointment edit requested',
                "A parent submitted an edit request for an appointment. New time: $when.", $appointment_id);
        }
        return true;
    }
    error_log('Error submitting edit request: ' . mysqli_error($conn));
    $_SESSION['error_message'] = 'Error submitting edit request.';
    return false;
}
...
<body class="bg-gray-50">
                                        ...
                                        <div class="appointment-item">
                                            ...
                                            <?php if (!empty($appointment['pending_modification_id'])): ?>
                                                <div class="mt-3 text-xs text-amber-700 font-medium">
                                                    Edit pending doctor approval
                                                    <?php if (!empty($appointment['pending_new_date']) && !empty($appointment['pending_new_time'])): ?>
                                                        <div class="text-amber-600 mt-1 font-normal">
                                                            Requested: <?php echo date('M j, Y', strtotime($appointment['pending_new_date'])); ?>
                                                            at <?php echo date('h:i A', strtotime($appointment['pending_new_time'])); ?>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
    ...
    <div id="editAppointmentModal" class="fixed inset-0 modal-backdrop hidden z-50 flex items-center justify-center p-4">
        <div class="modal-content bg-white rounded-xl shadow-2xl max-h-screen overflow-y-auto">
            <div class="p-4 md:p-6 lg:p-8">
                <div class="text-center mb-6 md:mb-8">
                    <h2 class="text-2xl md:text-3xl font-inter font-bold text-gray-800 mb-2">Request Appointment Edit</h2>
                    <p class="text-gray-600 responsive-text">
                        Submit a change request. The doctor will review and approve before the appointment is updated.
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full">
            <div class="p-6">
                ...
                <p class="text-sm text-gray-600 mb-4">
                    Your appointment will not be cancelled immediately — the assigned doctor will review your request and confirm.
                </p>
                <form method="POST">
