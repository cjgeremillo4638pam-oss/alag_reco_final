...
// Pending appointment modification requests addressed to this doctor — joined
// with patient and parent info for the approval UI.
function get_pending_modification_requests($conn, $doctor_id) {
    $doctor_id = intval($doctor_id);
    $requests = [];
    $sql = "SELECT amr.*, a.appointment_date as appt_current_date, a.appointment_time as appt_current_time,
                   a.type as appt_current_type, a.reason as appt_current_reason,
                   p.first_name as patient_first, p.last_name as patient_last,
                   u.first_name as parent_first, u.last_name as parent_last
            FROM appointment_modification_requests amr
            JOIN appointments a ON a.id = amr.appointment_id
            JOIN patients p ON p.id = a.patient_id
            JOIN users u ON u.id = p.parent_id
            WHERE amr.status = 'PENDING' AND a.doctor_id = ?
...
// Pending parent-initiated cancellation requests assigned to this doctor.
function get_pending_cancellation_requests($conn, $doctor_id) {
    $doctor_id = intval($doctor_id);
    $requests = [];
    $sql = "SELECT a.*, p.first_name as patient_first, p.last_name as patient_last,
            ...
            JOIN patients p ON p.id = a.patient_id
            JOIN users u ON u.id = p.parent_id
            WHERE a.doctor_id = ? AND a.status = 'CANCELLATION_REQUESTED'
            ORDER BY a.cancellation_requested_at DESC";
    $stmt = mysqli_prepare($conn, $sql);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "i", $doctor_id);
        mysqli_stmt_execute($stmt);
...
// Approve a parent-initiated cancellation request: only the assigned doctor
// can act on it. Marks the appointment CANCELLED and notifies the parent.
function approveCancellationRequest($conn, $data, $doctor_id) {
    ...
    $check = mysqli_prepare($conn,
        "SELECT id, patient_id, cancellation_requested_by, cancellation_reason, appointment_date, appointment_time
         FROM appointments WHERE id = ? AND doctor_id = ? AND status = 'CANCELLATION_REQUESTED'");
    mysqli_stmt_bind_param($check, "ii", $appointment_id, $doctor_id);
    ...
    $upd = mysqli_prepare($conn,
        "UPDATE appointments SET status = 'CANCELLED', cancelled_at = NOW(), updated_at = NOW() WHERE id = ?");
    mysqli_stmt_bind_param($upd, "i", $appointment_id);
    ...
    $msg = "Your cancellation request for the appointment on $when has been approved by the doctor.";
    if (function_exists('send_appointment_notification') && !empty($row['cancellation_requested_by'])) {
        send_appointment_notification($conn, intval($row['cancellation_requested_by']), $title, $msg, $appointment_id);
    }
    return ['success' => true, 'message' => 'Cancellation request approved'];
}

// Reject a parent-initiated cancellation request: appointment goes back to
// CONFIRMED so the slot stays valid. Doctor's note (if any) is appended.
function rejectCancellationRequest($conn, $data, $doctor_id) {
    $appointment_id = intval($data['appointment_id'] ?? 0);
    ...
    $check = mysqli_prepare($conn,
        "SELECT id, cancellation_requested_by, appointment_date, appointment_time
         FROM appointments WHERE id = ? AND doctor_id = ? AND status = 'CANCELLATION_REQUESTED'");
    mysqli_stmt_bind_param($check, "ii", $appointment_id, $doctor_id);
    mysqli_stmt_execute($check);
    $row = mysqli_fetch_assoc(mysqli_stmt_get_result($check));
    ...
    $note_clause = ' Cancellation rejected by doctor.' . ($review_note !== '' ? ' Note: ' . $review_note : '');
    $upd = mysqli_prepare($conn,
        "UPDATE appointments SET status = 'CONFIRMED', notes = CONCAT(COALESCE(notes, ''), ?), cancellation_requested_by = NULL, cancellation_requested_at = NULL, updated_at = NOW() WHERE id = ?");
    mysqli_stmt_bind_param($upd, "si", $note_clause, $appointment_id);
    if (!mysqli_stmt_execute($upd)) {
    ...
    $msg = "The doctor declined your cancellation request for $when." . ($review_note !== '' ? " Note: $review_note" : '');
    if (function_exists('send_appointment_notification') && !empty($row['cancellation_requested_by'])) {
        send_appointment_notification($conn, intval($row['cancellation_requested_by']), $title, $msg, $appointment_id);
    }
...
// Approve a parent-initiated edit: applies the requested date/time/type/reason
// to the appointments table and marks the modification request APPROVED.
function approveModificationRequest($conn, $data, $doctor_id) {
    ...
    $sel = mysqli_prepare($conn,
        "SELECT amr.*, a.doctor_id, a.status as appt_status FROM appointment_modification_requests amr
         JOIN appointments a ON a.id = amr.appointment_id
         WHERE amr.id = ? AND amr.status = 'PENDING' AND a.doctor_id = ?");
    mysqli_stmt_bind_param($sel, "ii", $request_id, $doctor_id);
    mysqli_stmt_execute($sel);
    $req = mysqli_fetch_assoc(mysqli_stmt_get_result($sel));
    if (!$req) {
        return ['success' => false, 'message' => 'Modification request not found'];
    }

    // Re-check slot conflict at approval time (someone else may have booked it).
    $conf = mysqli_prepare($conn,
        "SELECT id FROM appointments WHERE doctor_id = ? AND appointment_date = ? AND appointment_time = ? AND status IN ('SCHEDULED','CONFIRMED') AND id != ?");
    mysqli_stmt_bind_param($conf, "issi", $doctor_id, $req['new_date'], $req['new_time'], $req['appointment_id']);
    mysqli_stmt_execute($conf);
    if (mysqli_num_rows(mysqli_stmt_get_result($conf)) > 0) {
        return ['success' => false, 'message' => 'That time is no longer available — reject the request and ask the parent to pick another time.'];
    }

    $upd = mysqli_prepare($conn,
        "UPDATE appointments SET appointment_date = ?, appointment_time = ?, type = COALESCE(NULLIF(?, ''), type), reason = ?, updated_at = NOW() WHERE id = ?");
    mysqli_stmt_bind_param($upd, "ssssi",
        $req['new_date'], $req['new_time'], $req['new_type'], $req['new_reason'], $req['appointment_id']);
    if (!mysqli_stmt_execute($upd)) {
        return ['success' => false, 'message' => 'Failed to apply changes'];
    }

    $finish = mysqli_prepare($conn,
        "UPDATE appointment_modification_requests SET status = 'APPROVED', reviewed_by = ?, reviewed_at = NOW() WHERE id = ?");
    mysqli_stmt_bind_param($finish, "ii", $doctor_id, $request_id);
    mysqli_stmt_execute($finish);

    $when = date('M j, Y', strtotime($req['new_date'])) . ' at ' . date('h:i A', strtotime($req['new_time']));
    if (function_exists('send_appointment_notification')) {
        send_appointment_notification($conn, intval($req['requested_by']),
            'Edit request approved',
            "Your appointment has been rescheduled to $when.",
            intval($req['appointment_id']));
    }
    return ['success' => true, 'message' => 'Modification approved and applied'];
}

// Reject a parent-initiated edit: appointment is left unchanged, the request
// is marked REJECTED, and the parent is told why.
function rejectModificationRequest($conn, $data, $doctor_id) {
    $request_id = intval($data['request_id'] ?? 0);
    $review_note = trim($data['review_note'] ?? '');
    if ($request_id <= 0) {
        return ['success' => false, 'message' => 'Invalid request'];
    ...
    $sel = mysqli_prepare($conn,
        "SELECT amr.*, a.doctor_id FROM appointment_modification_requests amr
         JOIN appointments a ON a.id = amr.appointment_id
         WHERE amr.id = ? AND amr.status = 'PENDING' AND a.doctor_id = ?");
    mysqli_stmt_bind_param($sel, "ii", $request_id, $doctor_id);
    mysqli_stmt_execute($sel);
    ...
    $upd = mysqli_prepare($conn,
        "UPDATE appointment_modification_requests SET status = 'REJECTED', reviewed_by = ?, reviewed_at = NOW(), review_notes = ? WHERE id = ?");
    mysqli_stmt_bind_param($upd, "isi", $doctor_id, $review_note, $request_id);
    if (!mysqli_stmt_execute($upd)) {
        return ['success' => false, 'message' => 'Failed to reject request'];
    }

    if (function_exists('send_appointment_notification')) {
        $msg = 'The doctor declined your edit request. Your original appointment time stands.'
            . ($review_note !== '' ? " Note: $review_note" : '');
        send_appointment_notification($conn, intval($req['requested_by']),
            'Edit request declined', $msg, intval($req['appointment_id']));
    }
...
function recordVaccination($conn, $data, $doctor_id) {
    ...
    // Required fields per clinic policy: every administered dose must be
    // traceable, so lot/manufacturer/site and the next-dose date are mandatory.
    if ($lot_number === '') {
    ...
    if ($next_due_date === '' || !strtotime($next_due_date)) {
        return ['success' => false, 'message' => 'Next Dose Due is required'];
    }
...
function add_medical_record($conn, $data, $doctor_id) {
    ...
    // Vitals are now required — every medical record must capture them.
    if (!isset($data['temperature']) || $data['temperature'] === '') {
        return ['success' => false, 'message' => 'Temperature (°C) is required'];
    }
    if ($blood_pressure === '') {
        return ['success' => false, 'message' => 'Blood Pressure is required'];
    }
    if (!isset($data['heart_rate']) || $data['heart_rate'] === '') {
        return ['success' => false, 'message' => 'Heart Rate (BPM) is required'];
    }
    if (!isset($data['height']) || $data['height'] === '') {
        return ['success' => false, 'message' => 'Height (cm) is required'];
    }
    ...
    if ($heart_rate < 20 || $heart_rate > 300) {
    ...
    if (!preg_match('/^\d{1,3}\s*\/\s*\d{1,3}$/', $blood_pressure)) {
...
function update_doctor_profile($conn, $data, $doctor_id) {
    ...
    $phone_raw      = preg_replace('/\s+/', '', $data['phone'] ?? '');
    ...
    // Phone is required and must be a valid Philippine mobile number.
    // Accept 09XXXXXXXXX, +639XXXXXXXXX, 639XXXXXXXXX, or the bare 9XXXXXXXXX
    // (as typed in the +63-prefixed UI field). All are normalized to the
    // 10-digit "9XXXXXXXXX" form for storage.
    if ($phone_raw === '') {
    ...
    if (preg_match('/^\+?63(9\d{9})$/', $phone, $m)) {
        $phone = $m[1];
    } elseif (preg_match('/^0(9\d{9})$/', $phone, $m)) {
        $phone = $m[1];
    }
    if (!preg_match('/^9\d{9}$/', $phone)) {
        return ['success' => false, 'message' => 'Phone must be a valid Philippine mobile (e.g., 09XXXXXXXXX or +639XXXXXXXXX).'];
    }
    if ($specialization === '') {
        ...
        if (isset($_POST['action'])) {
            ...
            switch ($action) {
                ...
                case 'approve_cancellation':
                    $response = approveCancellationRequest($conn, $_POST, $current_user['id']);
                    break;

                case 'reject_cancellation':
                    $response = rejectCancellationRequest($conn, $_POST, $current_user['id']);
                    break;

                case 'approve_modification':
                    $response = approveModificationRequest($conn, $_POST, $current_user['id']);
                    break;

                case 'reject_modification':
                    $response = rejectModificationRequest($conn, $_POST, $current_user['id']);
                    break;
...
<body class="flex">
    ...
    <div class="flex-1 main-content overflow-auto">
        ...
        <div id="appointments-section" class="section-content p-6 hidden">
            ...
            <?php
            $pending_mods = get_pending_modification_requests($conn, $current_user['id']);
            $pending_cancels = get_pending_cancellation_requests($conn, $current_user['id']);
            if (!empty($pending_mods) || !empty($pending_cancels)):
            ?>
            <div class="card card-hover p-6 mb-6 border-l-4 border-amber-400">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-inter font-bold text-gray-800">
                        Pending Approval Requests
                        <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                            <?php echo count($pending_mods) + count($pending_cancels); ?>
                        </span>
                    </h2>
                </div>

                <?php if (!empty($pending_cancels)): ?>
                <div class="mb-4">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Cancellation Requests</h3>
                    <div class="space-y-3">
                        <?php foreach ($pending_cancels as $cr): ?>
                        <div class="p-4 bg-red-50 border border-red-100 rounded-lg">
                            <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-2">
                                <div class="flex-1">
                                    <div class="font-medium text-gray-800">
                                        <?php echo htmlspecialchars($cr['patient_first'] . ' ' . $cr['patient_last']); ?>
                                        <span class="text-xs text-gray-500">(parent: <?php echo htmlspecialchars($cr['parent_first'] . ' ' . $cr['parent_last']); ?>)</span>
                                    </div>
                                    <div class="text-sm text-gray-600 mt-1">
                                        Scheduled for <?php echo date('M j, Y', strtotime($cr['appointment_date'])); ?>
                                        at <?php echo date('g:i A', strtotime($cr['appointment_time'])); ?>
                                        — <?php echo htmlspecialchars($cr['type']); ?>
                                    </div>
                                    <?php if (!empty($cr['cancellation_reason'])): ?>
                                    <div class="text-sm text-gray-700 mt-2">
                                        <strong>Reason:</strong> <?php echo htmlspecialchars($cr['cancellation_reason']); ?>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                <div class="flex space-x-2 shrink-0">
                                    <button onclick="approveCancellation(<?php echo (int)$cr['id']; ?>)"
                                            class="bg-green-600 hover:bg-green-700 text-white text-sm font-medium px-3 py-1.5 rounded-md">
                                        Approve
                                    </button>
                                    <button onclick="rejectCancellation(<?php echo (int)$cr['id']; ?>)"
                                            class="bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium px-3 py-1.5 rounded-md">
                                        Reject
                                    </button>
                                </div>
                            </div>
                ...
                <div>
                    <h3 class="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Edit / Reschedule Requests</h3>
                    <div class="space-y-3">
                        <?php foreach ($pending_mods as $mr): ?>
                        <div class="p-4 bg-blue-50 border border-blue-100 rounded-lg">
                            <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-2">
                                <div class="flex-1">
                                    <div class="font-medium text-gray-800">
                                        <?php echo htmlspecialchars($mr['patient_first'] . ' ' . $mr['patient_last']); ?>
                                        <span class="text-xs text-gray-500">(parent: <?php echo htmlspecialchars($mr['parent_first'] . ' ' . $mr['parent_last']); ?>)</span>
                                    </div>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 text-sm">
                                        <div>
                                            <div class="text-xs text-gray-500 uppercase">Current</div>
                                            <div class="text-gray-700">
                                                <?php echo date('M j, Y', strtotime($mr['appt_current_date'])); ?>
                                                at <?php echo date('g:i A', strtotime($mr['appt_current_time'])); ?>
                                            </div>
                                            <div class="text-xs text-gray-500">Type: <?php echo htmlspecialchars($mr['appt_current_type'] ?: 'N/A'); ?></div>
                                        </div>
                                        <div>
                                            <div class="text-xs text-gray-500 uppercase">Requested</div>
                                            <div class="text-gray-800 font-medium">
                                                <?php echo date('M j, Y', strtotime($mr['new_date'])); ?>
                                                at <?php echo date('g:i A', strtotime($mr['new_time'])); ?>
                                            </div>
                                            <div class="text-xs text-gray-500">Type: <?php echo htmlspecialchars($mr['new_type'] ?: 'unchanged'); ?></div>
                                        </div>
                                    </div>
                                    <?php if (!empty($mr['new_reason'])): ?>
                                    <div class="text-sm text-gray-700 mt-2">
                                        <strong>Notes:</strong> <?php echo htmlspecialchars($mr['new_reason']); ?>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                <div class="flex space-x-2 shrink-0">
                                    <button onclick="approveModification(<?php echo (int)$mr['id']; ?>)"
                                            class="bg-green-600 hover:bg-green-700 text-white text-sm font-medium px-3 py-1.5 rounded-md">
                                        Approve
                                    </button>
                                    <button onclick="rejectModification(<?php echo (int)$mr['id']; ?>)"
                                            class="bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium px-3 py-1.5 rounded-md">
                                        Reject
                                    </button>
                                </div>
                        ...
                        <div>
                            ...
                            <div class="flex">
                                ...
                                <input type="tel" name="phone" required pattern="^9\d{9}$" maxlength="10"
                                       value="<?php
                                           $__phone = $current_user['phone'] ?? '';
                                           // Display only the local 10-digit portion (strip +63 / 0 prefix) so the
                                           // pattern matches what the user sees in the field.
                                           if (preg_match('/^\+?63(9\d{9})$/', $__phone, $m)) { $__phone = $m[1]; }
                                           elseif (preg_match('/^09(\d{9})$/', $__phone, $m)) { $__phone = '9' . $m[1]; }
                                           echo htmlspecialchars($__phone);
                                       ?>"
                                       ...
                                       title="Philippine mobile: 10 digits starting with 9 (e.g., 09171234567 or +639171234567)">
                            </div>
                            <p class="text-xs text-gray-500 mt-1">Philippine mobile number. Accepted formats: 09XXXXXXXXX or +639XXXXXXXXX.</p>
                        </div>
            ...
            <div class="bg-white rounded-lg shadow-xl">
                ...
                <div class="p-6">
                    <form id="prescriptionForm" onsubmit="handlePrescriptionForm(event)">
                            ...
                            <div>
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <select id="dosage_unit" name="dosage_unit" required
                                            class="w-1/3 px-2 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                        ...
                        <script>
                        // Stitch the unit dropdowns back into the existing dosage/frequency/duration
                        // fields the backend already consumes. Keep the hidden fields in sync on
                        // every change so they're populated regardless of which submit handler runs
                        // first (the form's inline onsubmit fires before any external listener).
                        (function() {
                            function syncDosage() {
                                var da = document.getElementById('dosage_amount').value.trim();
                                var du = document.getElementById('dosage_unit').value;
                                document.getElementById('dosage').value = (da && du) ? (da + ' ' + du) : '';
                            }
                            function syncFrequency() {
                                document.getElementById('frequency').value = document.getElementById('frequency_unit').value;
                            }
                            function syncDuration() {
                                var ta = document.getElementById('duration_amount').value.trim();
                                var tu = document.getElementById('duration_unit').value;
                                document.getElementById('duration').value = (ta && tu) ? (ta + ' ' + tu + (parseInt(ta,10) === 1 ? '' : 's')) : '';
                            }
                            ['input','change'].forEach(function(ev) {
                                document.getElementById('dosage_amount').addEventListener(ev, syncDosage);
                                document.getElementById('dosage_unit').addEventListener(ev, syncDosage);
                                document.getElementById('frequency_unit').addEventListener(ev, syncFrequency);
                                document.getElementById('duration_amount').addEventListener(ev, syncDuration);
                                document.getElementById('duration_unit').addEventListener(ev, syncDuration);
                            });
                            // Also sync on submit in capture phase so we beat the inline onsubmit
                            // attribute even if the user never typed in the dropdowns.
                            var f = document.getElementById('prescriptionForm');
                            if (f) f.addEventListener('submit', function() {
                                syncDosage(); syncFrequency(); syncDuration();
                            }, true);
                        })();
                        </script>
    ...
    // ── Cancellation / Modification request approval ──────────────────────────
    async function submitRequestAction(action, payload, confirmMessage) {
        if (confirmMessage) {
            const ok = typeof window.appConfirm === 'function'
                ? await window.appConfirm('Confirm', confirmMessage, null, { confirmText: 'Yes' })
        ...
        try {
            ...
            if (typeof window.showToast === 'function') {
                window.showToast(data.message || (data.success ? 'Done' : 'Error'), data.success ? 'success' : 'error');
            } else {
    ...
    function approveCancellation(appointmentId) {
        submitRequestAction('approve_cancellation', { appointment_id: appointmentId },
            'Approve this cancellation? The appointment will be cancelled and the parent notified.');
    }
    function rejectCancellation(appointmentId) {
        const note = prompt('Optional note for the parent (why is the cancellation being declined?):', '') || '';
        submitRequestAction('reject_cancellation', { appointment_id: appointmentId, review_note: note });
    }
    function approveModification(requestId) {
        submitRequestAction('approve_modification', { request_id: requestId },
            'Approve these changes? The appointment will be updated to the requested date/time.');
    }
    function rejectModification(requestId) {
        const note = prompt('Optional note for the parent (why is the edit being declined?):', '') || '';
        submitRequestAction('reject_modification', { request_id: requestId, review_note: note });
    }
