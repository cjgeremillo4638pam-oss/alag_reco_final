...
// Idempotent migration: pending appointment modification requests. Parent-
// initiated edits land here in PENDING state and only apply to the appointments
// table once the assigned doctor approves the request. Survives a missing table
// without breaking older deploys.
@mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `appointment_modification_requests` (
    `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
    `appointment_id` int(10) UNSIGNED NOT NULL,
    `requested_by` int(10) UNSIGNED NOT NULL,
    `new_date` date DEFAULT NULL,
    `new_time` time DEFAULT NULL,
    `new_type` varchar(32) DEFAULT NULL,
    ...
    PRIMARY KEY (`id`),
    KEY `idx_amr_appointment` (`appointment_id`),
    KEY `idx_amr_status` (`status`),
    KEY `idx_amr_requested_by` (`requested_by`)
